...
/*   Updated: 2024/07/13 17:05:35 by rmarzouk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_simple_cmd *new_cmd_node(t_cmd_limits *cmd)
{
	...
	new->next = NULL;
	new->prev = NULL;
	new->in_num = check_redir(cmd, REDIR_IN , HERE_DOC);// number of redir in 
	new->out_num = check_redir(cmd, REDIR_OUT, DREDIR_OUT);// number of redir out
	new->pipe_flag = 0;// how to check pipe flag ?? will set it later
	new->fd.in = -1;// execution part
	new->fd.out = -1;// execution part
	new->redir_in = set_redir(cmd, REDIR_IN_FILE, HERE_DOC_LIMITER,  new->in_num);// fill array with in redir
	new->redir_out = set_redir(cmd, REDIR_OUT_FILE, DREDIR_OUT_FILE, new->out_num);// fill array with out redir
	return (new);
}
